...
元のドキュメントはここにあります: [TypeInferencerリポジトリ](https://github.com/kekyo/TypeInferencer)
...
（日本語で記述してから、機械翻訳で英語に翻訳して手直ししています。そのため、英語版で表現に不適な箇所があるかもしれません。明らかにおかしな文法などは、PRなどで助けてもらえると非常に嬉しいです。）

---

## 型推論の概要

`TypeInferencer` は、コンピューター言語処理系で使われる「型推論」を、平易かつコンパクトに実装したものです。このような処理は、言語処理系の内部で実装されていて、我々が最もなじみのある `F#`、`OCaml`、`Haskell`、`Java`など、静的型システムを持つ言語の処理系が実装していますし、動的型システムでも、処理の最適化の為に、何らかの型推論を行っているかもしれません。

「型推論」とは、式の型を明示することなく、処理系によって型を自動的に計算して示すことです。例えば:
...
* 注意: F#の疑似コードと、実際の`TypeInferencer`を使うコードとを区別出来るように、疑似コードには、`[*]`を目印として示します。

人間がコードを記述する上では (1) のように型を明示しませんが、型推論器が推論することによって、(2) のように、式の項それぞれにどのような型が適用されうるのかを、自動的に計算・導出できます。

型推論器には様々な手法が考案されていますが、`TypeInferencer` は、型推論の導入で代表的な `Algorithm W`と`Algorithm M`という手法を実装していて、使い分ける事が出来ます。
...
4. `Code generator`: 構文木から、独立したコードを生成します。ターゲットで直に動作するコードを生成します。一般的には「コンパイル」に相当します。
...
これは、過不足なく、完全なF#のすべての機能を網羅しています。そこには、`Lexer`、`Parser`、`Type checker`、`Code generator`の公開APIが全てが含まれていて、個別に使用したり、組み合わせて使用する事が出来ます。あなたのアプリケーションに組み込んで使用する事も可能でしょう。
...
恐らくは、`TypeInferencer`のような素朴な型システムの構造を理解できれば、F#コンパイラサービスのような、より実践的なメタプログラミングを利用できるようになるでしょう。

---

## TypeInferencerの構文木

構文木のノードは以下のように定義されています:
...
まずは、最も理解しやすくて簡単な、リテラル値の型を推論してみましょう。コード中のリテラル値は、このノードで表現します。以下は、`123`という整数値を定義して、それを推論したものです:
...
この式は例外が発生します。実は、この構文木を推論することは出来ません。理由を考えてみて下さい。今日は眠れないかもしれません...
...
```fsharp
// [*] 正しい定義
let rec f x: int = f x   // 結果の型が推論出来ないため、仮にintとする
in
  f 123
```
...
`let`で実現できない理由として、束縛する式内に現れる自由変数のスコープが、`let`の外にあるためです。`ELet`で示した通り、束縛変数を参照できるスコープは、`let ... in ...`の`in`の後です。F#の`let rec`はこの制限を緩和します。
